...
//        System.out.println(talNumbers);
        ...
        //List<String> allPeopleNames=MockData.getPeople()
        //        .stream().forEach(person -> person.getFirstName());


        Map<String, Long> countPeopleNames = MockData.getPeople()
                .stream()
                .collect(Collectors.groupingBy(Person::getFirstName, Collectors.counting()));
        System.out.println(countPeopleNames);

        List<Integer> birthYears=new ArrayList<>();
        for (Person item:MockData.getPeople()) {
            birthYears.add(LocalDateTime.now().getYear()- item.getAge());
        }
        Map<Integer,Long> countBirthYears=MockData.getPeople()
                .stream()
                .collect(Collectors.groupingBy(Person::getAge,Collectors.counting()));
        int year=LocalDateTime.now().getYear();
        for (Map.Entry<Integer,Long> item: countBirthYears.entrySet()) {
